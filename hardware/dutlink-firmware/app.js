...
async function checkLatestRelease() {
  ...
  try {
    ...
    const bootloaderAsset = releaseData.assets.find(
      (asset) =>
        asset.name.includes("dfu-bootloader") && asset.name.endsWith(".bin"),
    );
    const applicationAsset = releaseData.assets.find(
      (asset) =>
    ...
    document.getElementById("bootloader-size").textContent = formatBytes(
      bootloaderAsset.size,
    );
    document.getElementById("application-size").textContent = formatBytes(
      applicationAsset.size,
    );
    ...
    bootloaderBinary = await downloadBinary(
      bootloaderAsset.browser_download_url,
    );

    addLog("Downloading application binary");
    applicationBinary = await downloadBinary(
      applicationAsset.browser_download_url,
    );
...
async function flashFirmware() {
  if (!dfu || !bootloaderBinary || !applicationBinary) {
    addStatus(
      "❌ Not ready to flash. Please check release and connect device first.",
      "error",
    );
    return;
  }
  ...
  try {
    ...
    addStatus(
      `Successfully flashed DUTLink firmware ${releaseData.tag_name}`,
      "success",
    );
