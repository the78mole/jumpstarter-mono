...
use crate::version;
use crate::{ctlpins::CTLPins, usbserial::*};
...
pub type ShellType =
    UShell<USBSerialType, StaticAutocomplete<N_COMMANDS>, LRUHistory<512, 10>, 512>;
...
pub fn handle_shell_commands<L, S, P>(
    shell: &mut ShellType,
        ...
        match result {
            Ok(Some(ushell_input::Command((cmd, args)))) => {
                ...
                match cmd {
                    ...
                    "console" => {
                        handle_console_cmd(&mut response, args, shell_status);
                    }
                    "monitor" => {
                        handle_monitor_cmd(&mut response, args, shell_status);
                    }
                    "meter" => {
                        handle_meter_cmd(&mut response, args, shell_status, power_meter);
                    }
                    "storage" => {
                        handle_storage_cmd(&mut response, args, storage);
                    }
                    "power" => {
                        handle_power_cmd(&mut response, args, ctl_pins, config);
                    }
                    "send" => {
                        handle_send_cmd(&mut response, args, send_to_dut);
                    }
                    "set" => {
                        handle_set_cmd(&mut response, args, ctl_pins);
                    }
                    "set-config" => {
                        handle_set_config_cmd(&mut response, args, config);
                    }
                    "get-config" => {
                        handle_get_config_cmd(&mut response, args, config);
                    }
                    "status" => {
                        handle_status_cmd(&mut response, args, shell_status);
                    }
                    "version" => {
                        version::write_version(&mut response);
                    }
    ...
    B: Write,
{
    ...
    if usage {
        write!(
            response,
            "usage: set-config name|tags|storage|usb_storage value"
        )
        .ok();
    }
}
    ...
    B: Write,
{
    ...
    if args == "name" {
        ...
        write!(
            response,
            "usage: get-config [name|tags|json|usb_console|power_on|power_off|power_rescue]"
        )
        .ok();
    }
}
    ...
    B: Write,
{
    if args == "" {
        write!(
            response,
            "Monitor: {}, Meter: {}",
            shell_status.monitor_enabled, shell_status.meter_enabled
        )
        .ok();
    } else {
