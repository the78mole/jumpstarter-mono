...
def test_drivers_energenie(httpserver: HTTPServer):
    ...
    httpserver.expect_request("/login.html", method="POST", data="pw=1").respond_with_data(
        "Login successful"
    )  # Defaults to status 200

    # 2. Response for turning ON switch 1 - Match raw data string
    httpserver.expect_request("/", method="POST", data="cte1=1").respond_with_data(
        "Switch turned ON"
    )  # Defaults to status 200

    # 3. Response for turning OFF switch 1 - Match raw data string
    httpserver.expect_request("/", method="POST", data="cte1=0").respond_with_data(
        "Switch turned OFF"
    )  # Defaults to status 200
