...
get_latest_version() {
    ...
    latest_version=$(echo "${output}" | grep "^jumpstarter-all (" | sed 's/jumpstarter-all (\([^)]*\)).*/\1/')
...
install_jumpstarter() {
    local source="$1"
    local index_url
    index_url=$(get_index_url "${source}")
    ...
    local version
    if ! version=$(get_latest_version "${index_url}"); then
    ...
    # shellcheck source=/dev/null  # activate script is generated by venv
